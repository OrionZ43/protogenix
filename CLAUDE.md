...
- For Spotify and Yandex Music it resolves the metadata, then searches YouTube and downloads the match. An album or playlist from either goes through one shared path, `_importCollection` (`ImportCollection` / `ImportTrack` in `importer/domain/import_collection.dart`), and becomes a playlist of the same name. Which video counts as a match is decided by `youtube_match.dart`: same title, same artist, same version, with the exact duration and the artist's own channel first. If the best pick of the «Artist - Title» search isn't from the artist's channel, a second «topic» search runs and both result lists are ranked together (`findYoutubeUpload`). A track without a match is skipped rather than replaced by a cover or a live version (`known-issues.md`). Spotify has no API access at all: a track comes from the page meta tags, an album or playlist from the `open.spotify.com/embed/…` page (`spotify_page.dart`), and that page gives **only the first 99 tracks of a playlist** — the import sheet says so under the URL field and the summary repeats it (`known-issues.md`). Yandex Music goes through `api.music.yandex.net` without a token (`yandex_music.dart`: track, album and playlist links on any `music.yandex.*` domain); the API answers only from countries where Yandex Music works, so behind a foreign VPN exit the import reports 451 — see `known-issues.md`.
