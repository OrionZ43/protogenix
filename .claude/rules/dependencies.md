...
- `.github/dependabot.yml`: каждый день в 06:00 по Минску Dependabot проверяет pub.dev и открывает PR. `youtube_explode_dart` идёт отдельным PR (группа `youtube`), всё остальное — одним общим (`other`). Крупные версии (semver-major) не предлагаются, `audiotags` не трогается совсем (раздел ниже). Раз в неделю — версии GitHub Actions в `ci.yml`.
