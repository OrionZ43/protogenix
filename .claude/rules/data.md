...
- **Треки из Яндекс Музыки и Spotify** (`_importCollection`, общий путь для обоих): id — id ролика YouTube, `title` — название из источника с версией в скобках, `artist` — все исполнители через «, », `album` — альбом трека, а без него название плейлиста (у треков плейлиста Spotify альбома нет вовсе, поэтому там всегда название плейлиста). По этим трём полям повторный импорт узнаёт уже скачанное без поиска на YouTube (`ImportedTracksIndex`, `imported_tracks_index.dart`, с 2026-09-13; для Spotify — с 2026-09-18). Меняешь, что импорт пишет в эти поля, — меняй и ключ, иначе повторный импорт снова пойдёт искать каждый трек.
