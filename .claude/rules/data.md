...
- **Настройки интерфейса** — `settings.json` в папке данных (`AppSettingsStore`, `core/services/app_settings_store.dart`); сейчас там только `lyricsLinesOnly`. Эквалайзер хранится отдельно, в `equalizer.json`.
