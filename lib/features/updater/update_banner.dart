...
class _UpdateBannerState extends ConsumerState<UpdateBanner>
  ...
  // В initState, а не ленивым полем: ленивый контроллер впервые создавался
  // в dispose(), если «Что нового» ни разу не показывали, — у уже снятого
  // виджета (assert «deactivated widget's ancestor»; нашёл тест баннера)
  ...
  Widget build(BuildContext context) {
    ...
    // Приложение рисует под строкой состояния (edge-to-edge, main.dart), а
    // баннер стоит в оболочке самым верхним — отступ от неё он делает сам,
    // вкладкам под ним его убирает UpdateBannerLayout. В 1.0.0 и 1.1.0
    // отступа не было, и на телефоне баннер заезжал под часы и значки.
    // Поля — снаружи стекла: иначе размытие выходило за края карточки.
    return Padding(
      padding:
          EdgeInsets.fromLTRB(16, 8 + MediaQuery.paddingOf(context).top, 16, 0),
      child: GlassPanel(
...
/// Баннер над содержимым вкладок — во всех трёх оболочках (app_shell.dart).
/// Приложение рисует под строкой состояния, и каждый экран отступает от неё
/// сам (SafeArea). Под видимым баннером этот отступ уже занят баннером —
/// вкладкам он не нужен, иначе была бы двойная щель.
class UpdateBannerLayout extends ConsumerWidget {
  const UpdateBannerLayout({super.key, required this.child});

  final Widget child;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final visible = ref.watch(updateProvider.select((s) => s.isVisible));
    return Column(
      children: [
        const UpdateBanner(),
        Expanded(
          // Всегда один и тот же виджет: иначе при появлении баннера вкладки
          // пересоздались бы и потеряли прокрутку и выделение
