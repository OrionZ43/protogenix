        ...
        child: DecoratedBox(
          decoration: BoxDecoration(
            gradient: LinearGradient(
              ...
              colors: [Colors.black, Colors.black.withAlpha(190), Colors.transparent],
...
class _ActiveLineWidgetState extends ConsumerState<_ActiveLineWidget>
  ...
  final Map<LyricSyllable, ({double scale, double yOffset, double glow})> _values = {};

  // ── Бас-резонанс: горизонтальный «толчок» активной строки ─────────────────
  final LyricSpring _resonanceSpr = LyricSpring(
    initial: 0.0, dampingRatio: 0.35, frequency: 5.0,
  );
  ...
  void _onTick(Duration elapsed) {
    ...
    for (final entry in _springs.entries) {
      ...
      if (sylDur <= 0) {
        ...
        syllableTimeScale = ((lineTimeScale - sylStart) / sylDur).clamp(0.0, 1.0);
      }
        ...
        children: widget.line.words.map((word) {
          return Row(
            ...
            children: word.syllables.map((syl) {
              final vals = _values[syl] ??
                  (scale: 1.0, yOffset: 0.0, glow: 0.0);
...
class _SyllableWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final color       = Color.lerp(Colors.white.withAlpha(110), Colors.white, glowAlpha)!;
...
class _EmphasizedSyllable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final rawSylTs  = ((lineTs - sylStart) / (sylEnd - sylStart)).clamp(0.0, 1.0);
    ...
    return Row(
      ...
      children: List.generate(letters.length, (li) {
        ...
        final letterTs = ((timeAlpha - lStart) / (lEnd - lStart)).clamp(0.0, 1.0);
...
class _AmbientPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _blob(canvas,
      center: Offset(size.width * 0.88, size.height * (0.72 + 0.08 * (1 - pulse))),
      radius: r2,
